...
export class baseCanvasModel extends DOMWidgetModel {
  defaults() {
    return {
      ...super.defaults(),
      _model_name: baseCanvasModel.model_name,
      _model_module: baseCanvasModel.model_module,
      _model_module_version: baseCanvasModel.model_module_version,
      _view_name: baseCanvasModel.view_name,
      _view_module: baseCanvasModel.view_module,
      _view_module_version: baseCanvasModel.view_module_version,
    };
  }
  ...
  private _forEachView(callback: (view: baseCanvasView) => void) {
    for (const view_id in this.views) {
      this.views[view_id].then((view: baseCanvasView) => {
        callback(view);
      });
    }
  }
  static model_name = 'baseCanvasModel';
  static model_module = MODULE_NAME;
  static model_module_version = MODULE_VERSION;
  static view_name = 'baseCanvasView'; // Set to null if no view
  ...
  views: Dict<Promise<baseCanvasView>>;
}

export class baseCanvasView extends DOMWidgetView {
  render(): void {
  ...
  private _mouseDown = (e: MouseEvent): void => {
    ...
    if (e.button === 1 || e.button === 2) {
      this._panStartX =
        this._Sx + (mouseX * this.intrinsicZoom) / this.userZoom;
      this._panStartY =
        this._Sy + (mouseY * this.intrinsicZoom) / this.userZoom;
      this.panning = true;
      e.preventDefault();
    }
  };
  ...
  private drawImageScaled(): void {
    ...
    this.displayContext.fillRect(0, 0, 100, 100);
